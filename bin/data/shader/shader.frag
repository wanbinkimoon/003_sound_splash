...
void main() {
  ...
  vec3 color = vec3(.9, .4, .6);
  vec3 color2 = vec3(.8, .4, random(vec2(0, 1)));
  ...
  v.x = fbm(u + vec2(u_time, 1.2));
  v.y = fbm(u + vec2(u_time, 2.3));
  
  vec2 z = vec2(0.);
  z.x = fbm(v + vec2(5.1, 1.2));
  ...
  color = mix(color,
              vec3(0.1, colorG , 0.3),
              clamp(f * u_sound, 0.0, 1.0));
  
  color = mix(color,
              vec3(u_sound * .34, 0.6, 0.2),
  ...
  color2 = mix(color2,
               vec3(color2R * u_sound, 0.2, .1),
               clamp(length(sin(s.y * u_sound)), .25 , 1.0));
  
  color2 = mix(color,
               vec3(0.1, 0.3, .6),
               clamp(length(sin(z.y * u_sound)), .2 , .8));
